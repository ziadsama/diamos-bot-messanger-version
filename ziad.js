...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("ZIAD (WHITE S.) ❯ ") + chalk.hex("#00ffff")("fixed by ZIAD (WHITE S.)"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ ziad (WHITE S.) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`ZIAD`, '[ ZIAD (WHITE S.)) ]');
        logger('Hey, thank you for using this Bot', '[ ZIAD (WHITE S.) ]');
        logger(`ZIAD`, '[ ZIAD (WHITE S.)) ]');
...
cron.schedule('0 0 0 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
     list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("✨نوم هنيك اصدقائي 💖...تحيات دياموس و مطوره", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 7 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("💖السهر غير مفيد لصحتكم ادهبو للنوم🤕 ..دياموس", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 20 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("صباح الخير اصدقائي ✨ تحيات دياموس و مطوره💖", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
